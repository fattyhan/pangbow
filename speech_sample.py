...
speech_key, service_region = "87fbb2b171d14588b376b86bcd5ef150", "eastus"
...
def speech_recognize_keyword_from_microphone():
    ...
    model = speechsdk.KeywordRecognitionModel("/Applications/workspace/KWSDemo/samples/python/console/final_midfa.table")
    ...
    def recognized_cb(evt):
        ...
        if evt.result.reason == speechsdk.ResultReason.RecognizedKeyword:
            #print('识别唤醒: {}'.format(evt))
            print('%s %s' % ('用户:', '嘿，胖宝'))
            ms.text_to_speech('我在呢')
            print('%s %s' % ('胖宝:', '我在呢～'))#就位回复
            user_say = str(ms.speech_to_text())#唤起语音输入
            print('%s %s' % ('用户:', user_say))
            ai_say = str(ms.ask_chat_gpt(user_say))
            print('%s %s' % ('胖宝:', ai_say))#语音答复
            ms.text_to_speech(ai_say)
...
def speech_recognize_keyword_locally_from_microphone():
    ...
    # Creates an instance of a keyword recognition model. Update this to
    # point to the location of your keyword recognition model.
    model = speechsdk.KeywordRecognitionModel("final_lowfa.table")
...
def pronunciation_assessment_from_stream():
    ...
    # Replace with your own subscription key and service region (e.g., "westus").
    # Note: The sample is for en-US language.
    speech_config = speechsdk.SpeechConfig(subscription='baccbefe9beb482f8b9cd3e7401c0934', region='eastus')
